...
스파르탄 필터, 케미칼, 에어로넷, 기상데이터 함께 분석


'''2023.07.06~ Sujin'''
''' Descending Mass Concentration'''
...
class site:
    def __init__(self,site):
        if site == 'unist':
            self.inform = ['KRUL (14)', 'KRUL (8)','aeronet_unist','20200101_20221231_KORUS_UNIST_Ulsan'] 
                            #필터자료 번호 #이온자료 번호 #에어로넷 자료 폴더 #자료기간+모든파라미터 확보 필수
                            
        elif site == 'yonsei':
            self.inform = ['KRSE (12)', 'KRSE (10)','aeronet_yonsei','20190101_20221231_Yonsei_University'] 
                            #필터자료 번호 #이온자료 번호 #에어로넷 자료 폴더 #자료기간+모든파라미터 확보 필수

        elif site == 'seoul':
            self.inform = ['KRSE (12)', 'KRSE (10)','aeronet_seoul','20190101_20221231_Seoul_SNU'] 
site = site('yonsei')
...
def filter_station (file_name):
    ...
    f['end_date'] = end_list
    f['start_date'] = f.index                                      #인덱스를 start date로 지정
    return f
...
class Extract:
    def __init__(self):
        ...
        self.aae = To_D_Index.aae[['Absorption_AOD[440nm]','Absorption_AOD[870nm]','Absorption_AOD[1020nm]','Absorption_Angstrom_Exponent_440-870nm']]
        self.eae = To_D_Index.eae[['AOD_Extinction-Total[440nm]','AOD_Extinction-Total[870nm]','AOD_Extinction-Total[1020nm]','Extinction_Angstrom_Exponent_440-870nm-Total']]
...
class Merge:
    def __init__(self):
        self.time_index = pd.date_range(start = '2019-01-01', end = '2023-12-31', freq='1d', name='times')
...
#%% WEATHER : 일별로 데이터베이스 만들기
def Weather_base (path):
    weather = glob.glob(path)
    
    weather_base = []
    for i in range (0, np.size(weather)):
        globals()['weather_'+str(i)] = pd.read_csv(weather[i], sep=',', engine='python', parse_dates=True)
        weather_base.append(globals()['weather_'+str(i)]) #2019또는 2020년부터 최근까지의 기상데이터
    weather = pd.DataFrame(np.vstack(weather_base),columns=['site','name','time','temp','wind_v','wind_d','humid','insol','vis'])
    del weather[weather.columns[0]], weather[weather.columns[0]] #앞 두 칼럼 제거함
    weather['time'] = pd.to_datetime(weather['time'])
    weather = weather.set_index('time')
    weather = weather.astype('float')
    
    #wind를 따로 분리
    #wind = weather[['wind_v','wind_d']]
    
    #기상데이터 평균 및 최대최소 고르기 - 일자료 생성
    weather_daily = weather.resample('1d').mean()
    weather_max = weather.resample("d").max() #하루 중에 최대, 최소인 것 고르기
    weather_min = weather.resample("d").min()
    
    # temp_list = pd.DataFrame() #데이터프레임 그냥 만들기
    weather_daily['temp_min'] = weather_min['temp']  
    weather_daily['temp_max'] = weather_max['temp']
    return weather_daily, weather_max, weather_min
df_weather, df_weather_max, df_weather_min = Weather_base(f'C:/Users/PC/OneDrive - UNIST/SPARTAN/{site.inform[2]}/OBS_ASOS'+'*.csv')

#%% invalid 기간 제거 안 된 원본
df_aeronet_org = df_aeronet.copy()
df_filter_org = df_filter.copy()                                               

#%% 유효한 데이터 모아서 9일자료들 완전히 합치기
def Make_full_df (df_filter, df_chemical, df_weather, df_aeronet):
    
    #####스파르탄 invalid 기간 제거하기
    for i in range (0, np.size(df_filter.index)):
        if df_filter['start_date'][i]+timedelta(days = 8) != df_filter['end_date'][i]:
            ...
            df_chemical[df_chemical.columns[0]].iloc[i] = np.nan                   #chemical 데이터도 동일하게
            
    
    #####그 외의 invalid 기간 - 정전 등 '유니스트'
    # 2023년 1월 12일 정전 - 삭제필요
    # 2023년 4월 25일 관측 시작한 필터 012의 1번째 period 삭제필요
    if site.inform[0].split(' ')[0] == 'KRUL':
        ...
        for d in range (0, np.size(day_invalid)):
            ...
            df_chemical[df_chemical.columns[0]].iloc[d_index] = np.nan                 #chemical 데이터도 동일하게
    
    
    #PM없는 행은 삭제
    df_filter = df_filter.dropna(subset = ['Filter PM2.5 mass'], axis=0)
    df_chemical = df_chemical.dropna(subset = [' Filter PM2.5 mass'], axis=0)
        
    #valid 기간만 추출하기
    day_list = df_filter.index
    
    #에어로넷 자료 9일씩 끊고 valid 기간에 대해서만 9일평균함
    a_list = []
    for a in range (len(day_list)):
        globals()['period_'+str(a)] = df_aeronet.loc[day_list[a]:day_list[a]+timedelta(days = 8)]
        a_list.append(globals()['period_'+str(a)].mean())                          
    ...
    #기상자료 9일씩 끊고 valid 기간에 대해서만 9일평균함
    w_list = []
    for w in range (len(day_list)):
        globals()['period_w_'+str(w)] = df_weather.loc[day_list[w]:day_list[w]+timedelta(days = 8)]
        w_list.append(globals()['period_w_'+str(w)].mean())                          
    df_weather_nine = np.vstack(w_list)
    df_weather_nine = pd.DataFrame(df_weather_nine)  
    df_weather_nine.columns = df_weather.columns
    df_weather_nine.index = pd.DatetimeIndex(day_list)    
    
    
    #필터, 에어로넷, 케미칼 칼럼들 모두 합치기
    df_database_total = pd.concat([df_filter[df_filter.columns[0:9]], df_aeronet_nine, df_weather_nine, df_chemical[df_chemical.columns[1:]]], axis=1)
    return df_database_total, day_list
df_database, day_list = Make_full_df(df_filter, df_chemical, df_weather, df_aeronet)

#%% 칼럼 이름 줄여서 쓰기
class F:
    def __init__(self):
        self.index = df_database.index
        self.pm = df_database['Filter PM2.5 mass']
        self.sul = df_database['Ammoniated Sulfate']
        self.nit = df_database['Ammonium Nitrate']
        self.sea = df_database['Sea Salt']
        self.soi = df_database['Fine Soil']
        self.bc = df_database['BC PM2.5']
        self.tra = df_database['Trace Element Oxides']
        self.res = df_database['Residual Matter']
F = F()
